window.StorageEngine=(()=>{
  ...
  async function save(state){state.meta={updatedAt:now(),deviceId,appVersion:'v10.1'};const {error}=await db.from('user_dashboards').upsert({user_id:USER,content:state},{onConflict:'user_id'});if(error)throw error;localStorage.setItem('jd-v101-backup',JSON.stringify(state));return state.meta.updatedAt}
  function local(){const b=localStorage.getItem('jd-v101-backup');return b?JSON.parse(b):null}
  function realtime(onData){return db.channel('jd-v101-'+USER).on('postgres_changes',{event:'*',schema:'public',table:'user_dashboards',filter:'user_id=eq.'+USER},p=>onData(normalizeV10(migrate(p.new?.content)))).subscribe()}
