window.StorageEngine=(()=>{
  ...
  function defaults(){
    ...
    return {version:'9.3',workspaces:seeds.map((x,i)=>({id:'ws-'+i,title:x[0],color:x[1],createdAt:now(),updatedAt:now(),deletedAt:null})),projects:[],tasks:[],googleCalendarUrl:'',meta:{}};
  }
  function migrate(c){
    if(!c)return defaults();
    if(c.version==='9.3'&&Array.isArray(c.workspaces))return c;
    const out={version:'9.3',workspaces:[],projects:[],tasks:[],googleCalendarUrl:c.googleCalendarUrl||'',meta:c.meta||{}};
  ...
  async function save(state){state.meta={updatedAt:now(),deviceId,appVersion:'v9.3'};const {error}=await db.from('user_dashboards').upsert({user_id:USER,content:state},{onConflict:'user_id'});if(error)throw error;localStorage.setItem('jd-v93-backup',JSON.stringify(state));return state.meta.updatedAt}
  function local(){const b=localStorage.getItem('jd-v93-backup');return b?JSON.parse(b):null}
  function realtime(onData){return db.channel('jd-v93-'+USER).on('postgres_changes',{event:'*',schema:'public',table:'user_dashboards',filter:'user_id=eq.'+USER},p=>onData(migrate(p.new?.content))).subscribe()}
