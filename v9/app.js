...
function normalizeWorkspaceOrder(){
  ...
  const ids=[...archive.querySelectorAll(':scope > .workspace')].map(el=>el.dataset.workspaceId);
...
function normalizeProjectOrder(){
  document.querySelectorAll('[data-project-container]').forEach(container=>{
    ...
    const ids=[...container.querySelectorAll(':scope > .project')].map(el=>el.dataset.projectId);
...
function normalizeTaskOrder(){
  document.querySelectorAll('[data-task-container]').forEach(container=>{
    ...
    const ids=[...container.querySelectorAll(':scope > .task-row')].map(el=>el.dataset.taskId);
...
document.addEventListener('DOMContentLoaded',async()=>{
  ...
  $('json-backup').onclick=()=>{const b=new Blob([JSON.stringify(state,null,2)],{type:'application/json'}),a=document.createElement('a');a.href=URL.createObjectURL(b);a.download='JD_Workspace_v10_1_Backup_'+new Date().toISOString().slice(0,10)+'.json';a.click();URL.revokeObjectURL(a.href)};
