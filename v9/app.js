...
function render(){CalendarUI.load(state.googleCalendarUrl);renderArchive();renderDetail();requestAnimationFrame(initSortable)}
...
function renderWorkspace(w,q){
  let ps=state.projects
    .filter(p=>p.workspaceId===w.id&&!p.deletedAt)
  ...
  return `<div class="workspace" data-workspace-id="${w.id}">
    <div class="workspace-head" onclick="selectItem('workspace','${w.id}')">
      <span class="drag-handle workspace-handle" onclick="event.stopPropagation()" title="순서 이동">≡</span>
      <button class="fold-button ${w.collapsed?'collapsed':''}" onclick="event.stopPropagation();toggleWorkspaceFold('${w.id}')" title="접기/펼치기">▼</button>
      <span class="workspace-dot" style="background:${w.color}"></span>
      <span class="workspace-name">${esc(w.title)}</span>
      <div class="workspace-actions"><button onclick="event.stopPropagation();openWorkspaceDialog('${w.id}')">수정</button></div>
    </div>
    <div class="workspace-projects ${w.collapsed?'collapsed':''}" data-project-container="${w.id}">
      ${ps.map(p=>renderProject(p)).join('')}
    </div>
  </div>`;
}
function renderProject(p){
  let ts=projectTasks(p.id).sort((a,b)=>(a.order||0)-(b.order||0));
  ...
  return `<div class="project" data-project-id="${p.id}">
    <div class="project-row ${selected.type==='project'&&selected.id===p.id?'active':''}" onclick="selectItem('project','${p.id}')">
      <span class="drag-handle project-handle" onclick="event.stopPropagation()" title="순서 이동">≡</span>
      <button class="fold-button ${p.collapsed?'collapsed':''}" onclick="event.stopPropagation();toggleProjectFold('${p.id}')" title="접기/펼치기">▼</button>
      <span class="project-dot"></span>
      <div class="project-main">
        <div class="project-title">${esc(p.title)}</div>
        <div class="meta">${all.length}개 Task</div>
      </div>
      <div class="progress-wrap" title="${progress}%"><div class="progress-bar" style="width:${progress}%"></div></div>
      <span class="progress-text">${progress}%</span>
    </div>
    <div class="project-children ${p.collapsed?'collapsed':''}" data-task-container="${p.id}">
      ${ts.map(t=>`<div class="task-row ${t.completed?'done':''} ${selected.type==='task'&&selected.id===t.id?'active':''}" data-task-id="${t.id}" onclick="selectItem('task','${t.id}')">
        <span class="drag-handle task-handle" onclick="event.stopPropagation()" title="순서 이동">≡</span>
...
function toggleWorkspaceFold(id){
  const w=state.workspaces.find(x=>x.id===id);
  if(!w)return;
  w.collapsed=!w.collapsed;
  w.updatedAt=StorageEngine.now();
  changed();
  renderArchive();
  initSortable();
}
function toggleProjectFold(id){
...
function renumber(items){
  items.forEach((item,index)=>item.order=(index+1)*100);
}
...
function normalizeWorkspaceOrder(){
  ...
  const ids=[...archive.children].filter(el=>el.classList.contains('workspace')).map(el=>el.dataset.workspaceId);
  const map=new Map(state.workspaces.map(x=>[x.id,x]));
  renumber(ids.map(id=>map.get(id)).filter(Boolean));
}

function normalizeProjectOrder(){
  document.querySelectorAll('[data-project-container]').forEach(container=>{
    const workspaceId=container.dataset.projectContainer;
    const ids=[...container.children].filter(el=>el.classList.contains('project')).map(el=>el.dataset.projectId);
    const map=new Map(state.projects.map(x=>[x.id,x]));
    const items=ids.map(id=>map.get(id)).filter(Boolean);
    items.forEach(item=>item.workspaceId=workspaceId);
    renumber(items);
  });
}

function normalizeTaskOrder(){
  document.querySelectorAll('[data-task-container]').forEach(container=>{
    const projectId=container.dataset.taskContainer;
    const ids=[...container.children].filter(el=>el.classList.contains('task-row')).map(el=>el.dataset.taskId);
    const map=new Map(state.tasks.map(x=>[x.id,x]));
    const items=ids.map(id=>map.get(id)).filter(Boolean);
    items.forEach(item=>item.projectId=projectId);
    renumber(items);
  });
}
...
function initSortable(){
  ...
  const archive=$('archive-list');
  if(archive){
    sortableInstances.push(new Sortable(archive,{
      animation:180,
      delay:0,
  ...
  document.querySelectorAll('[data-project-container]').forEach(container=>{
    sortableInstances.push(new Sortable(container,{
      group:{name:'jd-projects',pull:true,put:true},
      animation:180,
      delay:0,
      delayOnTouchOnly:true,
      touchStartThreshold:4,
      ...
      dragClass:'sortable-drag',
      onEnd:evt=>{
        const movedId=evt.item?.dataset?.projectId;
        const targetWorkspaceId=evt.to?.dataset?.projectContainer;
        const moved=state.projects.find(x=>x.id===movedId);
        if(moved && targetWorkspaceId)moved.workspaceId=targetWorkspaceId;
  ...
  document.querySelectorAll('[data-task-container]').forEach(container=>{
    sortableInstances.push(new Sortable(container,{
      group:{name:'jd-tasks',pull:true,put:true},
      animation:180,
      delay:0,
      delayOnTouchOnly:true,
      touchStartThreshold:4,
      ...
      onEnd:evt=>{
        ...
        const targetProjectId=evt.to?.dataset?.taskContainer;
        const moved=state.tasks.find(x=>x.id===movedId);
        if(moved && targetProjectId)moved.projectId=targetProjectId;
...
document.addEventListener('DOMContentLoaded',async()=>{
  ...
  $('workspace-form').onsubmit=e=>{e.preventDefault();const title=$('workspace-title').value.trim();if(!title)return;if(workspaceEditId){const w=state.workspaces.find(x=>x.id===workspaceEditId);w.title=title;w.color=$('workspace-color').value;w.updatedAt=StorageEngine.now()}else state.workspaces.push({id:uid('ws'),title,color:$('workspace-color').value,order:(state.workspaces.length+1)*100,collapsed:false,createdAt:StorageEngine.now(),updatedAt:StorageEngine.now(),deletedAt:null});$('workspace-dialog').close();changed();render()};
  ...
  $('project-form').onsubmit=e=>{e.preventDefault();const title=$('project-title').value.trim();if(!title)return;const siblings=state.projects.filter(x=>x.workspaceId===$('project-workspace').value&&!x.deletedAt);const p={id:uid('project'),workspaceId:$('project-workspace').value,title,order:(siblings.length+1)*100,collapsed:false,completed:false,completedAt:null,createdAt:StorageEngine.now(),updatedAt:StorageEngine.now(),deletedAt:null};state.projects.push(p);$('project-dialog').close();selected={type:'project',id:p.id};changed();render()};
  ...
  $('task-form').onsubmit=e=>{e.preventDefault();const title=$('task-title').value.trim();if(!title)return;const siblings=state.tasks.filter(x=>x.projectId===$('task-project').value&&!x.deletedAt);state.tasks.push({id:uid('task'),projectId:$('task-project').value,title,memo:$('task-memo').value,order:(siblings.length+1)*100,urgent:false,urgentAt:null,completed:false,completedAt:null,createdAt:StorageEngine.now(),updatedAt:StorageEngine.now(),deletedAt:null});$('task-dialog').close();changed();render()};
  ...
  $('json-backup').onclick=()=>{const b=new Blob([JSON.stringify(state,null,2)],{type:'application/json'}),a=document.createElement('a');a.href=URL.createObjectURL(b);a.download='JD_Workspace_v10_2_Backup_'+new Date().toISOString().slice(0,10)+'.json';a.click();URL.revokeObjectURL(a.href)};
...
window.toggleWorkspaceFold=toggleWorkspaceFold;window.toggleProjectFold=toggleProjectFold;
