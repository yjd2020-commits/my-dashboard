...
function selectItem(type,id){if(editing&&!confirm('수정 내용을 취소하고 이동할까요?'))return;editing=false;selected={type,id};renderArchive();renderDetail()}
function toggleUrgent(id){const p=state.projects.find(x=>x.id===id);p.urgent=!p.urgent;p.urgentAt=p.urgent?StorageEngine.now():null;p.updatedAt=StorageEngine.now();changed();render()}
