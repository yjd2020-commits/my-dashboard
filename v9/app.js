...
function renderProject(p){
  let ts=projectTasks(p.id);if(view==='active')ts=ts.filter(t=>!t.completed);if(view==='completedTasks')ts=ts.filter(t=>t.completed);
  return `<div class="project"><div class="project-row ${selected.type==='project'&&selected.id===p.id?'active':''}" onclick="selectItem('project','${p.id}')"><span class="project-dot"></span><div class="project-main"><div class="project-title">${esc(p.title)}</div><div class="meta">${ts.length}개 항목</div></div>${!p.completed?`<button class="urgent-star ${p.urgent?'on':''}" onclick="event.stopPropagation();toggleUrgent('${p.id}')">${p.urgent?'★':'☆'}</button>`:''}</div>${ts.map(t=>`<div class="task-row ${t.completed?'done':''} ${selected.type==='task'&&selected.id===t.id?'active':''}" onclick="selectItem('task','${t.id}')"><input class="check" type="checkbox" ${t.completed?'checked':''} onclick="event.stopPropagation()" onchange="toggleTask('${t.id}',this.checked)"><div class="task-main"><div class="task-title">${esc(t.title)}</div><div class="preview">${esc((t.memo||'메모 없음').replace(/\n/g,' '))}</div></div></div>`).join('')}</div>`;
}
...
function renderDetail(){
  ...
  if(selected.type==='workspace'){
    ...
    $('detail').innerHTML=`<div class="detail"><h2>${esc(o.title)}</h2><div class="pills"><span class="pill">${ps.length} Projects</span></div><div class="memo">Workspace는 프로젝트를 묶는 상위 분류입니다.</div><div class="danger"><button class="btn" onclick="trashWorkspace('${o.id}')">Workspace 삭제</button></div></div>`;
  }else if(selected.type==='project'){
    const w=state.workspaces.find(x=>x.id===o.workspaceId),all=projectTasks(o.id),open=all.filter(t=>!t.completed).length;
    $('detail').innerHTML=`<div class="detail"><h2>${esc(o.title)}</h2><div class="pills"><span class="pill">${esc(w?.title||'')}</span><span class="pill">${o.completed?'완료':'진행'}</span></div><div class="memo">전체 ${all.length}개 · 미완료 ${open}개</div><div class="row">${!o.completed?`<button class="btn" onclick="toggleUrgent('${o.id}')">${o.urgent?'Urgent 해제':'Urgent 지정'}</button>`:''}<button class="btn" onclick="toggleProjectComplete('${o.id}')">${o.completed?'다시 진행':'완료 처리'}</button></div><div class="danger"><button class="btn" onclick="trashProject('${o.id}')">프로젝트 삭제</button></div></div>`;
  }else if(selected.type==='task'){
    const p=state.projects.find(x=>x.id===o.projectId);
    $('detail').innerHTML=`<div class="detail"><h2>${esc(o.title)}</h2><div class="pills"><span class="pill">${esc(p?.title||'')}</span><span class="pill">${o.completed?'완료':'진행'}</span></div><div class="memo">${esc(o.memo||'메모 없음')}</div><div class="danger"><button class="btn" onclick="trashTask('${o.id}')">업무 삭제</button></div></div>`;
  }else{
    $('detail').innerHTML=`<div class="detail"><h2>${esc(o.title)}</h2><div class="row"><button class="btn" onclick="restore('${selected.type}','${o.id}')">복원</button><button class="btn" onclick="erase('${selected.type}','${o.id}')">영구 삭제</button></div></div>`;
  }
}
function renderUrgent(){
  ...
  $('detail').innerHTML=`<div class="detail"><h2>Urgent</h2>${urgent.length?urgent.map(p=>{const w=state.workspaces.find(x=>x.id===p.workspaceId),open=projectTasks(p.id).filter(t=>!t.completed).length;return `<div class="urgent-card" onclick="selectItem('project','${p.id}')"><strong>${esc(p.title)}</strong><div class="meta">${esc(w?.title||'')} · 미완료 ${open}</div></div>`}).join(''):'<div class="empty">Urgent 프로젝트가 없습니다.</div>'}</div>`;
}
...
function toggleUrgent(id){const p=state.projects.find(x=>x.id===id);if(!p||p.completed)return;p.urgent=!p.urgent;p.urgentAt=p.urgent?StorageEngine.now():null;p.updatedAt=StorageEngine.now();changed();render()}
...
document.addEventListener('DOMContentLoaded',async()=>{
  ...
  $('project-form').onsubmit=e=>{e.preventDefault();const title=$('project-title').value.trim();if(!title)return;const p={id:uid('project'),workspaceId:$('project-workspace').value,title,urgent:false,urgentAt:null,completed:false,completedAt:null,createdAt:StorageEngine.now(),updatedAt:StorageEngine.now(),deletedAt:null};state.projects.push(p);$('project-dialog').close();selected={type:'project',id:p.id};changed();render()};
  ...
  $('task-form').onsubmit=e=>{e.preventDefault();const title=$('task-title').value.trim();if(!title)return;state.tasks.push({id:uid('task'),projectId:$('task-project').value,title,memo:$('task-memo').value,completed:false,completedAt:null,createdAt:StorageEngine.now(),updatedAt:StorageEngine.now(),deletedAt:null});$('task-dialog').close();changed();render()};
  ...
  $('json-backup').onclick=()=>{const b=new Blob([JSON.stringify(state,null,2)],{type:'application/json'}),a=document.createElement('a');a.href=URL.createObjectURL(b);a.download='JD_Workspace_v9_2_Backup_'+new Date().toISOString().slice(0,10)+'.json';a.click();URL.revokeObjectURL(a.href)};
  await load(false);StorageEngine.realtime(data=>{if(!dirty){state=data;render();status('자동 반영')}else status('다른 기기 변경 감지')});
});
window.selectItem=selectItem;window.toggleUrgent=toggleUrgent;window.toggleTask=toggleTask;window.toggleProjectComplete=toggleProjectComplete;window.trashWorkspace=trashWorkspace;window.trashProject=trashProject;window.trashTask=trashTask;window.restore=restore;window.erase=erase;window.openWorkspaceDialog=openWorkspaceDialog;
