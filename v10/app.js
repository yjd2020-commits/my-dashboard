...
function initSortable(){
  ...
  if(archive && !archive._workspaceSortable){
    archive._workspaceSortable=new Sortable(archive,{
      animation:160,
  ...
  document.querySelectorAll('[data-project-container]').forEach(container=>{
    if(container._projectSortable)return;
    container._projectSortable=new Sortable(container,{
      group:'projects',
      animation:160,
...
function onProjectSort(evt){
  ...
  document.querySelectorAll('[data-project-container]').forEach(container=>{
    ...
    items.forEach(x=>x.workspaceId=container.dataset.projectContainer);
    renumber(items);
  });
  changed();
  render();
}
function onTaskSort(evt){
  ...
  document.querySelectorAll('[data-task-container]').forEach(container=>{
    ...
    const items=ids.map(id=>map.get(id)).filter(Boolean);
    items.forEach(x=>x.projectId=container.dataset.taskContainer);
    renumber(items);
  });
  changed();
  render();
...
document.addEventListener('DOMContentLoaded',async()=>{
  ...
  $('json-backup').onclick=()=>{const b=new Blob([JSON.stringify(state,null,2)],{type:'application/json'}),a=document.createElement('a');a.href=URL.createObjectURL(b);a.download='JD_Workspace_v10_Backup_'+new Date().toISOString().slice(0,10)+'.json';a.click();URL.revokeObjectURL(a.href)};
